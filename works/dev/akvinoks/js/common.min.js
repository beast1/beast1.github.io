...
pageHeader.dynamicThemeElements = Array.from(pageHeader.root.querySelectorAll('.page-header__dynamic-theme'));
pageHeader.setTheme = function(colorStr) {
  this.dynamicThemeElements.forEach(function(dynamicThemeElement) {
    dynamicThemeElement.dataset.theme = colorStr;
  });
};

function initScrollToId() {
  $("a[href*='#']").mPageScroll2id({
    offset: document.querySelector('.page-header').offsetHeight + 30,
    appendHash: true
  });
}

function initFullScreen() {
  var elems = Array.from(document.querySelectorAll('.fullscreen-element:not(.fullscreen-element_light)'));
  var elemsLight = Array.from(document.querySelectorAll('.fullscreen-element.fullscreen-element_light'));
  if (elems) {
    for (var i = 0; i < elems.length; i++) {
      var elem = elems[i];
      if (elem.offsetWidth <= document.documentElement.clientWidth && elem.offsetHeight <= document.documentElement.clientHeight) {
  ...
  if (elemsLight) {
    elemsLight.forEach(function(elemLight) {
      if (elemLight.offsetWidth <= document.documentElement.clientWidth && elemLight.offsetHeight <= document.documentElement.clientHeight) {
        elemLight.style.maxHeight = document.documentElement.clientHeight - 1 + 'px';
        elemLight.style.height = '100%';
      }
    });
  }
}
...
function initCustomPagination (parentSelector, sliderObj) {
  var swiperIndexOutput = document.querySelector(parentSelector + ' .slider-controls__pages-current');
  var swiperTotalSlidesNumberOutput = document.querySelector(parentSelector + ' .slider-controls__pages-total')
  function updPagination() {
    swiperIndexOutput.textContent = '0' + (sliderObj.activeIndex + 1);
  }

  swiperTotalSlidesNumberOutput.textContent = '/0' + sliderObj.slides.length;
  updPagination();

  sliderObj.on('slideChange', updPagination);
}

function initModal(selectorBtnOpen, selectorModal) {
  if (selectorBtnOpen !== undefined) {
    var btnOpen = document.querySelector(selectorBtnOpen);
    var modal   = document.querySelector(selectorModal);
    var btnExit = modal.querySelector('.page__exit');
    btnOpen.addEventListener('click', function () {
      modal.classList.remove('page_modal-hidden');
    });

    btnExit.addEventListener('click', function () {
      modal.classList.add('page_modal-hidden');
    });
  }
}

function initScrollSpy() {
  if (document.querySelector('body').dataset.scrollSpy) {
    var breakpointFirst  = document.querySelector('.project');
    var breakpointSecond = document.querySelector('.project__section-wrap_offer');

    function inWindow (elem, ballast) {
      var rect = elem.getBoundingClientRect();
      if (ballast) {
        return rect.top - ballast <= 0 && rect.bottom - ballast >= 0;
      } else {
    ...
    window.addEventListener('scroll', function () {
      if (inWindow(breakpointFirst, pageHeader.root.offsetHeight / 2)) {
        if (inWindow(breakpointSecond, pageHeader.root.offsetHeight / 2)) {
          pageHeader.setTheme('white');
        } else {
          pageHeader.setTheme('black');
        }
      } else {
        pageHeader.setTheme('white');
      }
...
initAccordions();
initFullScreen();
initModal('.page-header__burger .btns-item', '#menu');
initModal('.page-header__call-me', '#call-me');
initModal('.page-header__feedback .btns-item', '#feedback');
initScrollToId();